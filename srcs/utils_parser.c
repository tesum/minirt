...
void	parse_cam(char *str)
{
	...
	if (len_squared(g_scene.cams.direction) > 1.0)
...
void	parse_cylinder(char *str)
{
	...
	g_scene.cylinder[g_scene.c_cy].color = \
		mul_vec(g_scene.cylinder[g_scene.c_cy].color, 1 / 255.0);
	g_scene.c_cy += 1;
}
