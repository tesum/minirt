...
void	parse_sphere(char *str)
{
	...
	g_scene.sphers[g_scene.c_sp].color_light = mul_vec(g_scene.sphers[g_scene.c_sp].color_light, 1 / 255.0);
	g_scene.c_sp += 1;
}
...
void	parse_plane(char *str)
{
	...
	g_scene.plane[g_scene.c_pl].color = mul_vec(g_scene.plane[g_scene.c_pl].color, 1/255.0);
	g_scene.c_pl += 1;
}
