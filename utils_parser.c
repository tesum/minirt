...
void	parse_cylinder(char *str)
{
	...
	g_scene.cylinder[g_scene.c_cy].normalize_vector = parse_vec3(&str);
	vec3 = g_scene.cylinder[g_scene.c_cy].normalize_vector;
	if (vec3.x + vec3.y + vec3.z > 1.0)
		exit_error("Invalid 3d normalized vector cylinder", ERR_CREATE_CYLINDER);
	...
	g_scene.cylinder[g_scene.c_cy].height = ft_atof(&str);
	if (g_scene.cylinder[g_scene.c_cy].height < 0.0)
